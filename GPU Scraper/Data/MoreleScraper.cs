    ...
    public class MoreleScraper
    {
        public IEnumerable<Product> ScrapProducts()
        {
            ...
            foreach (var url in Urls)
            {
                ...
                {
                    var productName = string.Join(' ', product.QuerySelector(".cat-product-name > h2 > a").Attributes["title"].Value.ToString().Split(' ').Skip(2));
                    var productPrice = double.Parse(product.QuerySelector(".price-new").InnerText.ToString(nfi).Replace("zł", "").Replace("od", ""));
